          ...
          <div className={`mt-top-adjusted ${navbar ? 'adjust-down' : ''}`}>
            <div className="flex text-adainblack hover:text-adainwhite items-center justify-between py-5">
              <a href="/" className="flex items-center">
                <Image src="/images/adainwhiteicon.svg" width={40} height={20} alt="Avista Beach Resort Logo" />
   ...
   Products and services here, such that that users can switch to true or false ... */}
    ...
    {isDropdownOpen && (
      <div className="font-bold absolute py-8 my-4 text-sm  text-adainblack hover:text-adainwhite">
        <Link href="/quickmed-api-url" className="rounded-2xl mx-2 px-2 dropdown-link backdrop-blur-xl shadow-lg bg-adainyellow text-adainwhite hover:text-adainblack ">
         QuickMed
        </Link>
    ...
    <Link href="/navlinks/firebase-auth-page" className="sm:px-16 text-avista3 pr-10 mr-4 text-lg  lg:my-20" onClick={() => handleClick('checkout')}>
      Sign in
    </Link>
  </div>
</ul>
